...
namespace PitStriker.UI
{
    ...
    /// 1. Home Screen (Play, Maps, How to Play, Settings, Exit)
    /// 2. Play Mode Select (Local / Online)
    /// 3. Match Setup (Choose 2/3/4 Players, toggle Real Player vs AI Bot per slot)
    /// 4. Maps / Settings screens (reference stills parity)
    ...
    public partial class MenuManager : MonoBehaviour
    {
        ...
        public void ShowScreen(ScreenType screen)
        {
            ...
            {
                if (screen == ScreenType.Home || screen == ScreenType.ChoosePlayers || screen == ScreenType.PlayModeSelect || screen == ScreenType.OnlineMenu || screen == ScreenType.QuickMatch || screen == ScreenType.CreateMatch || screen == ScreenType.JoinMatch || screen == ScreenType.OnlineLobby || screen == ScreenType.OnlineResult || screen == ScreenType.Maps || screen == ScreenType.Settings || (screen == ScreenType.Rules && _rulesReturn != ScreenType.Pause))
                {
