...
namespace PitStriker.CameraSystem
{
    ...
    public class SmoothFollowCamera : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            // Skip follow updates when target has fallen below gameplay surface (respawn loop protection)
            if (_target != null && PitStriker.GameplayKit.SurfaceGroundConfig.Instance != null)
            {
                float top = PitStriker.GameplayKit.SurfaceGroundConfig.Instance.GetSurfaceTopWorldY();
                if (_target.position.y < top - 3f)
                    return;
            }
        }
