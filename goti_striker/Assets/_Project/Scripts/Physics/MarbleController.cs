...
namespace PitStriker.Physics
{
    ...
    public class MarbleController : MonoBehaviour
    {
        ...
        public void ApplyImpulse(Vector3 direction, float forceMagnitude, float maxPitch = 0.12f)
        {
            if (IsRetired || _rigidbody == null) return;

            // Upward pitch clamped (Ground ~0.08â€“0.12, Loft higher via caller)
            direction.y = Mathf.Clamp(direction.y, 0f, Mathf.Max(0.01f, maxPitch));
        ...
        private void FixedUpdate()
        {
            ...
            {
                Vector3 safe = new Vector3(0f, 0.3f, -5.5f);
                if (PitStriker.GameplayKit.GameplayOrigin.Instance != null)
                    safe = PitStriker.GameplayKit.GameplayOrigin.Instance.GetStartWorld();
                ResetPosition(safe);
