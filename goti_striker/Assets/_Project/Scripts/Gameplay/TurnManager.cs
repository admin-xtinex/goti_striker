...
namespace PitStriker.Gameplay
{
    ...
    public class TurnManager : MonoBehaviour
    {
            ...
            else
            {
                ...
                for (int i = 0; i < _players.Count; i++)
                {
                    ...
                    if (p.marble != null)
                    {
                        ...
                        if (rb != null) rb.isKinematic = false;
                        p.marble.Halt();
                        p.marble.ResetPosition(GetStartCenterWorld());
        ...
        {
            if (GameplayOrigin.Instance != null)
                return GameplayOrigin.Instance.GetPitWorld(pitNumber);
            switch (pitNumber)
            {
        ...
        Vector3 GetStartCenterWorld()
        {
            if (GameplayOrigin.Instance != null)
                return GameplayOrigin.Instance.GetStartWorld();
            return _startCenter;
        }

        Vector3 GetTeeWorldAfterPit(int conqueredPit)
        {
            if (GameplayOrigin.Instance != null)
            {
                var o = GameplayOrigin.Instance;
                if (conqueredPit == 1) return o.LocalToWorld(o.TeeAfterPit1Local);
                if (conqueredPit == 2) return o.LocalToWorld(o.TeeAfterPit2Local);
            }
            if (conqueredPit == 1) return new Vector3(0f, 0.3f, 4.5f);
            if (conqueredPit == 2) return new Vector3(0f, 0.3f, 18.0f);
            return GetStartCenterWorld();
        }
